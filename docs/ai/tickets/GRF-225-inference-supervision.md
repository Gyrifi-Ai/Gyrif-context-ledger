...
- [ ] `command.Stdout` and `command.Stderr` are wired to a reader that forwards each line to the `slog` logger at `debug` level with a `component: "llama-server"` attribute.
- [ ] `StartLlamaServer` takes a `*slog.Logger` parameter. It currently takes none — thread it through from `bootstrap`.
- [ ] Lines are read continuously and never block the child; a stalled reader must not deadlock llama-server on a full pipe. Use a bounded scanner goroutine per stream and drain to completion on exit.
- [ ] Extremely long lines are truncated rather than buffered without limit.
- [ ] When startup fails to become ready within the 45 s deadline, the **captured output is included in the returned error or logged at error level** — this is the change that makes model-load failures diagnosable.

**Supervision**

- [ ] A supervisor goroutine calls `command.Wait()` and records the exit.
- [ ] An unexpected exit is logged at `error` with the exit code and the last N captured stderr lines.
- [ ] The supervisor restarts the process with bounded exponential backoff (1s, 2s, 4s … capped at 60s), re-running the readiness poll after each start.
- [ ] Restarts are capped: after a configurable number of consecutive failures (`GYRIFI_INFERENCE_MAX_RESTARTS`, default 5), the supervisor stops trying and marks inference permanently unhealthy until the process is restarted. A crash-looping child must not be restarted forever.
- [ ] A successful readiness check resets the failure counter.
- [ ] Exit caused by intentional shutdown (`Stop()` or context cancellation) is **not** treated as a failure and does not trigger a restart. This distinction must be explicit, not inferred from the exit code.
- [ ] `Stop()` remains correct: `SIGTERM`, bounded wait, then kill. It must be safe to call while a restart is in flight, and must not race the supervisor. Verified under `-race`.
- [ ] No goroutine or file descriptor leaks across repeated restarts, verified by a test that cycles the process several times.

**Health**

- [ ] `LlamaServer` exposes `Healthy() bool` and `State() string` (`"ready" | "starting" | "restarting" | "failed" | "stopped"`).
- [ ] `GET /api/v1/system/status` reports the inference state, feeding the `health.inference` field defined in GRF-224. If GRF-224 has not landed, add the field here and note the overlap.
- [ ] An evaluation attempted while inference is not ready fails with `503 UNAVAILABLE` and the message "Evaluation is unavailable: the inference process is not running." — **not** a raw connection-refused error.
- [ ] A `503` from this cause must not be recorded as a failed check against the Proposal. An infrastructure outage is not evidence about the content. Verify no `checks` row is written.
- [ ] Studio's evidence panel (GRF-207) renders that state distinctly from a failed evaluation.

**General**

- [ ] No new dependencies.
- [ ] Behaviour is unchanged when `GYRIFI_EVALUATION_PROVIDER` is not `llamacpp` — no supervisor, no goroutines, no logs.
