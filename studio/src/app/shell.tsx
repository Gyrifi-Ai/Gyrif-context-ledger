...
export function Shell() {
  ...
  const pages = { ledgers: <LedgersPage />, changes: <ChangesPage />, proposals: <ProposalsPage proposalId={route.area === "proposals" ? route.id : undefined} />, releases: <ReleasesPage /> };
  return <ApplicationShell sidebar={<div className="flex h-full flex-row items-center gap-4 p-3 md:flex-col md:items-stretch md:gap-6 md:p-4"><a href="#ledgers" className="flex items-center gap-3 px-2"><span className="grid size-9 place-items-center rounded-md bg-primary font-semibold text-primary-foreground">G</span><span className="hidden md:block"><b className="block text-sm">Gyrifi</b><small className="text-[11px] uppercase tracking-wider text-muted-foreground">Context ledger</small></span></a><Nav route={route.area} ledgerId={ledgerId} /><div className="ml-auto flex items-center gap-3 md:ml-0 md:mt-auto md:block"><div className="hidden px-2 pb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground md:block">Active ledger</div><div className="overflow-visible rounded-md border border-border bg-card shadow-sm"><LedgerSwitcher /><div className="border-t border-border/60 bg-muted/40 px-3 py-2"><RuntimeStatus /></div></div></div></div>} topbar={<HeadChip ledgerId={ledgerId} />} banner={<ReachabilityBanner />} header={route.area === "releases" ? <PageHeader {...pageInfo.releases} /> : null}><ErrorBoundary key={route.area} fallback={(error, reset) => <div className="space-y-4"><ErrorState title="This page could not render" message="The rest of Studio is still available. Reset this page or choose another area." onRetry={reset} actionLabel="Reset page" /><CodeBlock value={error.message} language="error" /></div>}>{pages[route.area]}</ErrorBoundary></ApplicationShell>;
}
