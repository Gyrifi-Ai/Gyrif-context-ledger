...
describe("domain status tones", () => {
  it("maps every API status to its normative tone", () => {
    ...
    expect(intentTone("READY")).toBe("warning"); expect(intentTone("APPLYING")).toBe("warning"); expect(intentTone("VERIFYING")).toBe("warning"); expect(intentTone("FINALIZED")).toBe("success"); expect(intentTone("RECOVERY_REQUIRED")).toBe("danger"); expect(intentTone("ABANDONED")).toBe("neutral");
  });
});
